...
debug_name="webshop-QWEN_0.5B-4GPU-grpo-basic-medium-batch_size_mini"
python train.py --config-name _6_webshop.yaml trainer.project_name=webshop system.CUDA_VISIBLE_DEVICES='"0,1,2,3"' trainer.experiment_name=$debug_name $USE_GRPO $USE_BASE $USE_MEDIUM $USE_PPO_MINI_BATCH $USE_4_GPU_MODEL > logs/$debug_name.out 2> logs/$debug_name.err &
